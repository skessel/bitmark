...
public class KeyFingerprinter {
  ...
  /**
   * Hexadecimal characters to translate to when converting from a byte[] array to a hex string.
   */
   ...
   * This representation is still the one used by Triton and Manta as the key id.
   *
  ...
  /**
   * Starting with version 6.8 OpenSSH used the base64 encoded SHA256 of the serialized public key as
   * a fingerprint.
   *
   * @param keyPair The KeyPair to calculate the fingerprint of
  ...
  /**
   * Verifies that the given fingerprint matches the key. The fingerprint can be in any of the formats
   * used by OpenSSH including the pre-6.7 format ({@code
   * 9f:0b:50:ae:e3:da:f6:eb:b5:71:9a:69:ee:79:9e:c2}), or 6.8 format prefixed with the algorithm name
   * ({@code
  ...
  /**
   * Given a byte array, space it out with colons and lowercase each character to match the OpenSSH
   * format. Example output would be {@code
  ...
  static String colonify(final byte[] bytes) {
    requireNonNull(bytes, "byte array is null");
  ...
  /*
   * OpenSSH generally uses its own serialized representation of public keys. This is different from
   * the ASN.1 or X.509 format that may be returned by Java's getEncoded() method. Each key type
   * necessarily has its own format as well. In general the representation includes a name followed by
   * a series of arbitrarily large integers serialized as length+bytes. This is based on the work in
   * the node-sshpk project. The specific ordering of the encoded pieces (ie the exponent and modulus
   * for RSA) is also based on node-sshpk.
   */
  private static class SshEncoder {
     ...
     * sshpk parts: ['curve', 'Q']
     *
     * Unfortunately the ECDSA serialization is a bit quirky. Both the "name" and "curve name" strings
     * are are used, and they include the "key size" (ex: nistp256). A larger complication is that "Q"
     * (the elliptic curve point) is not a simple big int but a compound representation of the
     * coordinates. The is described in details in RFC 5656 and the "SEC 1: Elliptic Curve Cryptography"
     * <http://www.secg.org/sec1-v2.pdf> paper on which the RFD depends. Fortunately, the point
     * representation is the same as the ASN.1 representation used by Java, so we can let the standard
     * library do all of the bit twiddling and grab the appropriate bytes at the end.
     *
     * These details are summarized by https://security.stackexchange.com/a/129913
     */
    ...
    /*
     * The OpenSSH serialization format can in principle express a variety of types. Fortunately only
     * byte arrays (representing either strings or big integers) are required to represent public keys.
     * They are serialized as the length (requiring the unsigned int conversion) followed by the bytes.
     */
